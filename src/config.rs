...
impl Config {
    ...
    pub fn hash_password(&self, password: &String) -> String {
        ...
        let hashed_pass = a2.hash_password(password.as_bytes(), self.get_salt().expect("Could not retrieve salt from config")).expect("Could not hash new password");
        hashed_pass.serialize().to_string()
    }

    pub fn verify_password(&self, plain_password: &String, hash: &String) -> bool {
        let a2 = Argon2::default();
        match a2.verify_password(plain_password.as_bytes(), &PasswordHashString::parse(hash, argon2::password_hash::Encoding::B64).expect("Could not parse password hash").password_hash()) {
            Ok(_) => true,
            Err(_) => false,
        }
    }
